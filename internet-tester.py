...
def main():
	...
	conn.execute('''CREATE TABLE IF NOT EXISTS speed (timestamp TIMESTAMP PRIMARY KEY, 
	...
	if errors == b'':
		...
		# add internet down entry to database
		conn.execute("INSERT INTO results VALUES (?, ?, ?, ?, ?, ?, ?, ?)", 
						[ts.now().isoformat(), errno] + [None for i in range(6)])
